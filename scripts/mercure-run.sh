...
if [[ -z "$RUNNING_WEBAPP" ]];then
    singularity instance start $MERCURE_WEBAPP_SIF -B $MERCURE_DIR -B $JOBS_DIR -B $PIPELINES_DIR mercure-webappd
    success=$?
...
if [[ -z "$RUNNING_ROUTINE" ]];then
    singularity instance start $MERCURE_ROUTINE_SIF -B $MERCURE_DIR -B $JOBS_DIR -B $PIPELINES_DIR mercure-routined
    success=$?
    if [[ $success -ne 0 ]]; then
