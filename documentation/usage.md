...
Pour un pipeline donné, il est nécessaire de créer au moins un des deux dossiers `launchers` et `shared`, dont les fonctions respectives sont décrites dans les paragraphes suivants.
...
Dans le dossier `shared`, on trouve des fichiers `.sh`, qui permettent la réutilisation du code des pipelines.
