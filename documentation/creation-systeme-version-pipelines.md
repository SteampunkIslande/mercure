...
- Tout fichier commençant par `Snakefile` doit être versionné selon SemVer (Majeure.Mineure.Patch). Comme pour les actions, il ne peut y avoir qu'une seule révision associée.
- `$PIPELINE_NAME/config/*`: tout fichier dans le dossier config (là où doivent être stockés les fichiers de configuration pipeline-spécifiques) doit être versionné selon SemVer, et ne peut donc avoir qu'une seule et unique révision.
- Tous les autres fichiers (scripts helpers, profils de workflow) peuvent avoir autant de révisions que nécessaire.
